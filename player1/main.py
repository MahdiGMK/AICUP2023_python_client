import random , time
from player1.data import Map , ProxyMap , Genome , HuristicFunction
from player1.transition import MoveKind , Movement
import player1.transition as ts , player1.data as pd
...
def initializer(game: ClientGame):
    ...
    if (turn_number > 15) : 
        res = ts.miniMax(hr , 4 , playerId , [0,0,0] , 1 , 1 , 3) # 4,3
        for mv in res[1] : 
...
def turn(game: ClientGame):
    ...
    print(hr.calculateValue() , hr.viewDataForDbug())
    res = ts.miniMax(hr , 5 , playerId , [0 , 0 , 0] , 1 , 1 , 3) # optimize needed ...defualt 5 4 
    print(res[1])
    ...
    # move
    hr = HuristicFunction.makeNew(ProxyMap.makeCopy(prx) , playerId)
    res = ts.miniMax(hr , 4 , playerId , [0 , 0 , 0] , 0 , 1 , 2) # 4 2 
    for mv in res[1] :
